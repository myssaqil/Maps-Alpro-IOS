...
class JalurTercepatCari(JalurTercepatDijkstra):
    def get_rute(self, start, tujuan):
        distances, previous = self.dijkstra(start)
        if distances[tujuan] == float('inf'):
            return None, None
        ...
        return path, distances[tujuan]

    def tampilkan_rute(self, start, tujuan):
        path, jarak = self.get_rute(start, tujuan)
        if path is None:
            print(f"Tidak ada rute dari {start} ke {tujuan}")
        else:
            print(f"Rute tercepat dari {start} ke {tujuan}:")
            print(" -> ".join(path))
            print(f"Total jarak: {jarak} km")

jalur_tercepat = JalurTercepatCari()
for k in kota:
    jalur_tercepat.tambahkanKota(k)
jalur_tercepat.tambahkanJalan("Jakarta", "Bandung", 150)
jalur_tercepat.tambahkanJalan("Jakarta", "Semarang", 450)
jalur_tercepat.tambahkanJalan("Bandung", "Yogyakarta", 390)
jalur_tercepat.tambahkanJalan("Semarang", "Surabaya", 320)
jalur_tercepat.tambahkanJalan("Yogyakarta", "Surabaya", 430)

jalur_tercepat.tampilkan_rute("Jakarta", "Surabaya")